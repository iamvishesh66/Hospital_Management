...
class vei_nur:
    ...
    def __init__(self, parent, a, b):
        ...
        title = Label(self.f2, text="VIEW NURSE", font=("algerian", 20, "bold", "underline"), fg="dodger blue3"
        ...
        self.table = Treeview(self.f3, columns=("id", "name", "gender", "phno", "address", "wing", "roomalloted")
        ...
        btn4 = Button(self.f1, text="Refresh", font=("arial", 11, "bold"), bd=3
                      , command=lambda: vei_nur(self.nur_win1, 20, 40), fg="green")
    ...
    def onstart(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM nurse")
                result = dc.fetchall()
                self.table.delete(*self.table.get_children())
                for row in result:
                    self.table.insert("", END, values=row)
                if not result:
                    messagebox.showinfo("INFO", "NO DATA FOUND", parent=self.nur_win1)
        ...
        try:
            ...
            try:
                ...
                if self.v1.get() == "Name":
                    ...
                    dc.execute("SELECT * FROM nurse ORDER BY id ASC")
                elif self.v1.get() == "Descending Order":
                    dc.execute("SELECT * FROM nurse ORDER BY id DESC")
                result = dc.fetchall()
                self.table.delete(*self.table.get_children())
                for row in result:
                    self.table.insert("", END, values=row)
    ...
    def searcher(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM nurse WHERE name LIKE '%"+str(self.e1.get())+"%'")
                result = dc.fetchall()
                self.table.delete(*self.table.get_children())
                for row in result:
                    self.table.insert("", END, values=row)
    ...
    def inserter(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT img FROM nurse WHERE id=%s",id)
                res = dc.fetchone()
                for i in res:
                    self.del_img = i
                ...
                t1 = Label(self.f1, text="Nurse's Photo", font=("arial",10,"bold","italic","underline"),fg="purple").place(x=10,y=0)
