...
class del_doc:
    ...
    def __init__(self, parent, a, b):
        ...
        title = Label(self.f2, text="REMOVE DOCTOR", font=("algerian", 20, "bold", "underline"), fg="dodger blue3"
                      , bg="white").place(x=350, y=0)

        l1 = Label(self.f1, text="Id Of Doctor", background="gray95", font=("arial", 16)).place(x=0, y=5)
        ...
        self.btn3 = Button(self.f1, text="REFRESH", command=lambda: del_doc(self.doc_win1, 20, 40),
        ...
        self.table.column("#2", stretch=NO, minwidth=120, width=120)
    ...
    def savedata(self):
        ...
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT img FROM doctor WHERE id=%s", (self.e1.get()))
                res = dc.fetchone()
                for i in res:
                    self.del_img = i
                path = os.path.join("docimages//", self.del_img)
                os.remove(path)
                dc.execute("DELETE FROM doctor WHERE id =%s",
                           (self.e1.get()))
                d1.commit()
                messagebox.showinfo("SAVE", "Data Deleted Successfully", parent=self.doc_win1)
                del_doc(self.doc_win1,20,40)
    ...
    def fetcher(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM doctor WHERE name like '%"+str(self.e2.get())+"%'")
