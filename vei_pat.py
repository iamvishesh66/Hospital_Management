...
class vei_pat:
    ...
    def __init__(self, parent, a, b):
        ...
        title = Label(self.f2, text="VIEW PATIENT", font=("algerian", 20, "bold", "underline"), fg="dodger blue3"
        ...
        self.table = Treeview(self.f3, columns=("id", "name", "gender", "phno", "address", "roomalloted", "cname", "cphno")
        ...
        self.table.column("#6", stretch=NO, minwidth=130, width=130)
        ...
        btn4 = Button(self.f1, text="Refresh", font=("arial", 11, "bold"), bd=3
                      , command=lambda: vei_pat(self.pat_win1, 20, 40), fg="green")
        btn4.place(x=890, y=115)
    ...
    def onstart(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM patient")
                result = dc.fetchall()
        ...
        try:
            ...
            try:
                ...
                if self.v1.get() == "Name":
                    ...
                    dc.execute("SELECT * FROM patient ORDER BY id ASC")
                elif self.v1.get() == "Descending Order":
                    dc.execute("SELECT * FROM patient ORDER BY id DESC")
                result = dc.fetchall()
                self.table.delete(*self.table.get_children())
                for row in result:
                    self.table.insert("", END, values=row)
    ...
    def searcher(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM patient WHERE name LIKE '%"+str(self.e1.get())+"%'")
                result = dc.fetchall()
                self.table.delete(*self.table.get_children())
                for row in result:
                    self.table.insert("", END, values=row)
    ...
    def inserter(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT img FROM patient WHERE id=%s",id)
                res = dc.fetchone()
                for i in res:
                    self.del_img = i
                ...
                t1 = Label(self.f1, text="Patient's Photo", font=("arial",10,"bold","italic","underline"),fg="purple").place(x=10,y=0)
