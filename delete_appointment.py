...
class del_app:
    ...
    def __init__(self, parent, a, b):
        ...
        title = Label(self.f2, text="CANCEL APPOINTMENT", font=("algerian", 20, "bold", "underline"), fg="dodger blue3"
                      , bg="white").place(x=350, y=0)

        l1 = Label(self.f1, text="Appointment Id", background="gray95", font=("arial", 16)).place(x=0, y=5)
        ...
        self.btn3 = Button(self.f1, text="REFRESH", command=lambda :del_app(self.doc_win1, 20, 40), font=("arial", 14, "bold"), bd=5)
        ...
        self.table = Treeview(self.f3, columns=("ID", "NAME", "GENDER", "PHONE NUMBER", "REASON FOR VISIT", "DOCTOR PROVIDED",
                                                "DATE","TIME")
        ...
        self.table.heading("REASON FOR VISIT", text="REASON FOR VISIT")
        self.table.heading("DOCTOR PROVIDED", text="DOCTOR PROVIDED")
        ...
        self.table.column("#7", stretch=NO, minwidth=50, width=50)
        self.table.column("#8", stretch=NO, minwidth=50, width=50)
    ...
    def savedata(self):
        ...
        try:
            ...
            try:
                dc = d1.cursor()
                dc1 = d1.cursor()
                dc.execute("DELETE FROM appointment WHERE ID =%s",
                           (self.e1.get()))
                d1.commit()
                dc1.execute("DELETE FROM revisit WHERE id=%s", self.e1.get())
                d1.commit()
                messagebox.showinfo("SAVE", "Data Deleted Successfully", parent=self.doc_win1)
                del_app(self.doc_win1,20,40)
    ...
    def fetcher(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM appointment WHERE name like '%"+str(self.e2.get())+"%'")
