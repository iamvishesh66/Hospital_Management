...
class del_pat:
    ...
    def __init__(self, parent, a, b):
        ...
        title = Label(self.f2, text="REMOVE PATIENT", font=("algerian", 20, "bold", "underline"), fg="dodger blue3"
                      , bg="white").place(x=350, y=0)

        l1 = Label(self.f1, text="Id Of Patient", background="gray95", font=("arial", 16)).place(x=0, y=5)
        ...
        self.btn3 = Button(self.f1, text="REFRESH", command=lambda: del_pat(self.doc_win1, 20, 40),
        ...
        self.table = Treeview(self.f3, columns=("ID", "NAME", "GENDER", "PHONE NUMBER", "ADDRESS", "ROOM ALLOTED",
                                                "COMPANION'S NAME", "COMPANION'S PHONE NUMBER")
        ...
        self.table.heading("COMPANION'S NAME", text="COMPANION'S NAME")
        self.table.heading("COMPANION'S PHONE NUMBER", text="COMPANION'S PHONE NUMBER")
        self.table["show"] = "headings"

        self.table.column("#0", stretch=NO, minwidth=0, width=0)
        self.table.column("#1", stretch=NO, minwidth=30, width=30)
        self.table.column("#2", stretch=NO, minwidth=80, width=80)
        ...
        self.table.column("#7", stretch=NO, minwidth=150, width=150)
    ...
    def savedata(self):
        ...
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT img FROM patient WHERE id=%s", (self.e1.get()))
                res = dc.fetchone()
                for i in res:
                    self.del_img = i
                path = os.path.join("patimages//", self.del_img)
                os.remove(path)
                dc.execute("DELETE FROM patient WHERE ID =%s",
                           (self.e1.get()))
                d1.commit()
                messagebox.showinfo("SAVE", "Data Deleted Successfully", parent=self.doc_win1)
                del_pat(self.doc_win1,20,40)
    ...
    def fetcher(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT * FROM patient WHERE name like '%"+str(self.e2.get())+"%'")
