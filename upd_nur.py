...
class upd_nur:
    ...
    def __init__(self, parent, a, b):
        ...
        self.c_1 = Combobox(self.canvas1, values=("ID", "NAME", "WING", "ROOM ALLOTTED"), textvariable=self.v_1,
                            state="readonly"
        ...
        self.table = Treeview(self.f3, columns=("id", "name", "gender", "phonenumber", "address", "wing", "roomalloted")
        ...
        self.table.column("#6", stretch=NO, minwidth=80, width=80)
        ...
        l6 = Label(self.f1, text="Wing", background="gray95", font=("arial", 16)).place(x=2, y=430)
        l7 = Label(self.f1, text="Room Alloted", background="gray95", font=("arial", 16)).place(x=2, y=480)
        ...
        self.e5.place(x=200, y=480)

        self.btn1 = Button(self.f1, text="UPDATE", command=self.updatedata, font=("arial", 14, "bold"), bd=5)
        self.btn1.place(x=700, y=510)

        self.btn2 = Button(self.f1, text="REFRESH", command=lambda: upd_nur(self.update, 20, 40),
                           font=("arial", 14, "bold"), bd=5)
        self.btn2.place(x=850, y=510)
    ...
    def entryboxcreater(self):
        ...
        self.e_4 = Entry(self.canvas1, width=15, bg="white", font=("arial", 14, "italic"))

        self.v_2 = StringVar()
        l1 = Label(self.canvas1, font=("arial", 14), textvariable=self.v_2)
        l1.place(x=2, y=40)
    ...
    def showtext1(self):
        ...
        if not self.t_2.get():
            self.f1.focus_set()
            self.t_2.set("Enter Nurse's Name")
            self.e_2.config(textvariable=self.t_2, fg="grey")
        if not self.t_3.get():
            self.f1.focus_set()
            self.t_3.set("Enter Nurse's Wing")
    ...
    def showtext(self):
        ...
        if not self.t2.get():
            self.f1.focus_set()
            self.t2.set("Enter Nurse's Name")
            self.e2.config(textvariable=self.t2, fg="grey")
        if not self.t3.get():
            self.f1.focus_set()
            self.t3.set("Enter Nurse's Phone Number")
            self.e3.config(textvariable=self.t3, fg="grey")
        if not self.t4.get():
            self.f1.focus_set()
            self.t4.set("Allot Room To Nurse")
    ...
    def remtext(self, name):
        if name == "e1":
            ...
            if self.t4.get() == "Allot Room To Nurse":
                self.t4.set("")
            else:
    ...
    def searcher(self):
        try:
            ...
            try:
                dc = d1.cursor()
                if self.v_1.get() == "ID":
                    dc.execute("SELECT * FROM nurse WHERE id=%s", (self.e_1.get()))
                elif self.v_1.get() == "NAME":
                    dc.execute("SELECT * FROM nurse WHERE name like '%" + str(self.e_2.get()) + "%'")
                elif self.v_1.get() == "WING":
                    dc.execute("SELECT * FROM nurse WHERE wing like '%" + str(self.e_3.get()) + "%'")
                elif self.v_1.get() == "ROOM ALLOTTED":
                    dc.execute("SELECT * FROM nurse WHERE roomalloted like '%" + str(self.e_4.get()) + "%'")
    ...
    def inserter(self):
        ...
        name = selectedRow[1]
        wing = selectedRow[5]
        roomalloted = selectedRow[6]
        try:
            d1 = connect(host="localhost", user="root", password="", db="hospital")
            try:
                dc = d1.cursor()
                if self.v_1.get() == "ID":
                    dc.execute("select * from nurse where id=%s", id)
                elif self.v_1.get() == "NAME":
                    dc.execute("select * from nurse where name=%s", name)
                elif self.v_1.get() == "WING":
                    dc.execute("select * from nurse where wing=%s", wing)
                elif self.v_1.get() == "ROOM ALLOTTED":
                    dc.execute("select * from nurse where room alloted=%s", roomalloted)
                result = dc.fetchall()
                try:
                    for row in result:
        ...
        try:
            ...
            try:
                dc = d1.cursor()
                if self.v_1.get() == "ID":
                    dc.execute("SELECT img FROM nurse WHERE id=%s", (self.e_1.get()))
                elif self.v_1.get() == "NAME":
                    dc.execute("SELECT img FROM nurse WHERE name like '%" + str(self.e_2.get()) + "%'")
                elif self.v_1.get() == "SPECIALITY":
                    dc.execute("SELECT img FROM nurse WHERE speciality like '%" + str(self.e_3.get()) + "%'")
                elif self.v_1.get() == "WING":
                    dc.execute("SELECT img FROM nurse WHERE wing like '%" + str(self.e_4.get()) + "%'")
                res = dc.fetchone()
                for i in res:
                    self.del_img = i
                tkimage = PhotoImage(file="nurimages//" + str(self.del_img))
                a = tkimage.height()
                b = tkimage.width()
    ...
    def updatedata(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute(
                    "UPDATE nurse SET id=%s, name=%s, gender=%s, phonenumber=%s, address=%s, wing=%s, roomalloted=%s, img=%s WHERE id=%s",
                    (self.e1.get(), self.e2.get(), self.v1.get(), self.e3.get(),
                     self.e4.get("1.0", END), self.v3.get(),self.e5.get(),self.finalname + ".png", self.e1.get()))
                d1.commit()
                messagebox.showinfo("SAVE", "Data Updated Successfully", parent=self.update)
                upd_nur(self.update, 20, 40)
            except Exception as e:
                messagebox.showerror("ERROR", "Error in creation" + str(e), parent=self.update)
        except Exception as e:
            messagebox.showerror("ERROR", "Error in Connection" + str(e), parent=self.update)
    ...
    def imageupload(self):
        try:
            ...
            img.save("nurimages//" + self.finalname + ".png")
            tk_image = ImageTk.PhotoImage(img)
            a = tk_image.height()
            b = tk_image.width()
            tkimage = PhotoImage(file="nurimages//" + self.finalname + ".png")
            if a > 1000 and b > 1000:
                p1 = tkimage.subsample(15,15)
                self.imglabel.config(image=p1)
                self.imglabel.image = p1
