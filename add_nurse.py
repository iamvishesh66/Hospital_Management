...
class add_nur:
    ...
    def __init__(self, parent, a, b):
        ...
        self.nur_win1 = parent

        self.filename = ""

        self.f1 = Frame(self.nur_win1, height=600, width=1000, bg="gray95")
        self.f1.place(x=a, y=b)
        self.f1.focus()

        self.f2 = Frame(self.nur_win1, height=40, width=1000, bg="white")
        self.f2.place(x=20, y=-7)

        title = Label(self.f2, text="ADD NURSE", font=("algerian",20,"bold","underline"), fg="dodger blue3"
        ...
        l3 = Label(self.f1, text="Gender", background="gray95", font=("arial", 16)).place(x=200, y=120)
        l4 = Label(self.f1, text="Phone Number", background="gray95", font=("arial", 16)).place(x=0, y=170)
        l5 = Label(self.f1, text="Address", background="gray95", font=("arial", 16)).place(x=0, y=220)
        l6 = Label(self.f1, text="Wing", background="gray95", font=("arial", 16)).place(x=0, y=320)
        l7 = Label(self.f1, text="Room Allotted", background="gray95", font=("arial", 16)).place(x=0, y=390)
    ...
    def savedata(self):
        ...
        try:
            ...
            try:
                ...
                if self.filename == "":
                    img = Image.open(
                        "C:/Users/vishesh/PycharmProjects/Project/Hospital_Management/default_img1.png")
                    self.finalname = str(int(time.time()))
                    img.save("nurimages//" + self.finalname + ".png")
                dc = d1.cursor()
                dc.execute("insert into nurse values(%s,%s,%s,%s,%s,%s,%s,%s)",(self.e1.get(),self.e2.get(),self.v1.get()
                                                                                  ,self.e3.get(),self.e4.get("1.0", END),
                                                                                  self.v2.get()
                                                                                  ,self.e5.get(), self.finalname + ".png"))
                d1.commit()
                messagebox.showinfo("SAVE", "Data Saved Successfully", parent=self.nur_win1)
                add_nur(self.nur_win1,20,40)
            except Exception as e:
                messagebox.showerror("ERROR", "Error in creation" + str(e), parent=self.nur_win1)
        except Exception as e:
            messagebox.showerror("ERROR", "Error in Connection" + str(e), parent=self.nur_win1)
    ...
    def onstart(self):
        try:
            ...
            try:
                dc = d1.cursor()
                dc.execute("SELECT id FROM nurse")
                f = dc.fetchall()
                for i in f:
                    for j in i:
                        current_id = j
